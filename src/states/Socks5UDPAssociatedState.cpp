...
Socks5UDPAssociatedState::Socks5UDPAssociatedState(QUdpSocket * clientFacing,
    ...
    : SocksState(parent)
    , _clientFacing(clientFacing)
    , _remoteFacing(remoteFacing)
    , _lastClientUDPPort(0)
{
}
...
void Socks5UDPAssociatedState::handleSetAsNewState()
{
    ...
    connect(_clientFacing.data(),
            ...
            this,
            &Socks5UDPAssociatedState::handleClientReadyRead);
    connect(_remoteFacing.data(),
            &QUdpSocket::readyRead,
            this,
            &Socks5UDPAssociatedState::handleRemoteReadyRead);
...
void Socks5UDPAssociatedState::handleClientDatagram(QHostAddress senderHost, quint16 senderPort, QByteArray bytes)
{
    ...
    if (const auto parseResult = msg->parse(bytes, &parseErrorString); parseResult != SocksProtocolMessage::Success)
    {
...
{
    if (socket == nullptr || senderHost == nullptr || senderPort == nullptr || bytes == nullptr)
    {
